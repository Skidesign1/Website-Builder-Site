...
export function CanvasDroppable({ containers, overIndex, isDraggingNew, onDelete }) {
    ...
    return (
        <div
            ref={setNodeRef}
            className={cn("flex-1 overflow-y-scroll border border-dashed flex flex-col h-[100vh] overflow-auto", isOver && containers.length === 0 && "bg-muted/20")}
        >
                    ...
                    <SortableContext items={containers.map((container) => container.id)} strategy={rectSortingStrategy}>
                        {containers.map((container, index) => (
                            <div key={container.id} className="relative">
                                ...
                                <SortableContainer onDelete={onDelete} id={container.id} title={container.title} />
                            </div>
                        ))}
