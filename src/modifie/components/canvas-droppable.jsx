...
export function CanvasDroppable({ containers, overIndex, isDraggingNew, activeDroppableId }) {
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn("flex-1 flex flex-col h-full", isOver && containers.length === 0 && "bg-muted/20")}
    >
      <div className="flex-1 border p-8 min-h-full">
            ...
            {containers.map((container, index) => (
              <div key={container.id} className="relative">
                ...
                {isDraggingNew && overIndex === index && (
                  <div className="h-16 border-2 border-dashed border-primary bg-primary/10" />
                )}
